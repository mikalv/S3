 ...
 * `initiateMultipartUpload()` to 5an XML DOM string
...
function initiateMultipartUpload(authInfo, request, log, callback) {
    ...
    uploadId, cipherBundle, metadataStoreParams, callback) {
        const xmlParams = {
            bucketName,
            objectKey,
            uploadId,
    ...
    function _storetheMPObject(destinationBucket, corsHeaders) {
        ...
        if (config.backends.data === 'multiple') {
            multipleBackendGateway.createMPU(bucketName, objectKey,
            websiteRedirectHeader, backendInfoObj.controllingLC,
            (err, data) => {
                if (err) {
                    return callback(err);
                }
