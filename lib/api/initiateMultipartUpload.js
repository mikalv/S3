...
function initiateMultipartUpload(authInfo, request, log, callback) {
    ...
    function _storetheMPObject(destinationBucket, corsHeaders) {
        ...
        if (config.backends.data === 'multiple') {
            uploadIdObj = multipleBackendGateway.createMPU(bucketName,
            objectKey, websiteRedirectHeader, backendInfoObj.controllingLC);
        }
        // Generate uniqueID without dashes so that routing not messed up
        const scalUploadId = UUID.v4().replace(/-/g, '');

        metadataStoreParams.uploadId = uploadIdObj.uploadId || scalUploadId;
